...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::LoadPastEvents()
{
    if(QueryResult* queryResult = CharacterDatabase.Query("SELECT * FROM custom_td_events"))
    {
            ...
            if(queryResult)
            {
                ...
                if(!finished)
                {
                    if(QueryResult* queryResult = CharacterDatabase.PQuery("SELECT * FROM custom_td_players WHERE playerGUID = '%u'", playerGUID)){
                        ...
                        RecordLog("TowerDefense: 玩家GUID插入统计: [%u], 添加 [%u] 资源和未完成事件.", playerGUID,currentResources);
                    }
        ...
        RecordLog("TowerDefense: 未完成的事件删除,并将消息发送给玩家");
    }
    else if(!queryResult)
        RecordLog("TowerDefense: 未找到历史事件!");
}
/*
*  刷新生物NPC等
*/
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::OnUpdate(uint32 const diff)
{
    if(GetEventStatus() < TD_EVENT_STATUS_RUNNING) // 检测会话
        return;
    ...
    while (uint32 eventId = Events.ExecuteEvent())
    {
        ...
        case TD_EVENT_DATA_START_MUSIC_TIMER: // 播放背景音乐
            {
                Events.RescheduleEvent(TD_EVENT_DATA_START_MUSIC_TIMER, 240000); // 4分钟重复一次,修改记得确定音频轨道长度
            }
            break;
        ...
        case TD_EVENT_DATA_START_WAVE_MODE:
            {
                CountDown = 0;
                SetEventStatus(TD_EVENT_STATUS_LOADING); // 会话运行,BOSS NPC开始生成
                SendMessageToPlayer(TD_SYSTEM_MSG_WAVE_STARTING_GO);
                Events.CancelEvent(TD_EVENT_DATA_START_WAVE_MODE);
                RecordLog("TowerDefense: 会话ID: [%u] 正在运行中.", GetCurrentWaveId());
            }
            break;
        }
    }
...
/*
* 按照格式，可以生成怪物，添加会话，会话也可以添加怪物，移除等等
*/
...
/*
* 玩家点击 TD_ITEM();,开始计数，每秒向玩家播放一次，持续3秒，到0后会话被设置为true
*/
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::StartNextWave(uint32 timer)
{
    ...
    RecordLog("TowerDefense: 会话ID: [%u] 将在 %u 毫秒内运行.", GetCurrentWaveId(), timer);
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::StartEvent(Player* player, uint32 Action)
{
    ...
    SetupEventData();
    player->AddItem(34131, 1); // 项目添加给玩家
    player->SaveToDB(); // 如果崩溃了，将玩家数据保存到数据库
    ...
    RecordLog("TowerDefense: 玩家角色: [%s] 已启动塔防剧情. 事件ID: [%u]",player->GetName(), GetEventId());
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SpawnNextCreature(Creature* spawner)
{
    WaveInfo* wave = GetWaveById(GetCurrentWaveId());
    if(wave)
        wave->Spawn(GetUnits(), spawner);
    else
        sLog.outBasic("TowerDefense: 会话ID [%u] 没有加载正确的数据库内容.", GetCurrentWaveId());
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SpawnGuard(uint32 entry)
{
    if (!entry)
        return;
    Player *player = GetPlayer();
    ...
    eventFilter << " AND (eventEntry IS NULL "; //对象没整清楚，不动了
    ...
    if (!queryResult)
    {  
        SendMessageToPlayer(TD_SYSTEM_MSG_NO_NEARBY_PLATFORM);  // 没有平台来放置守卫
        return;
    }
    do
    {
    ...
    if (!found)
    {
        SendMessageToPlayer(TD_SYSTEM_MSG_NEARBY_PLATFORM_NOT_EXIST); // 找不到附近的gameobject
        return;
    }
    ...
    guard->X = objectX;
    guard->Y = objectY;
    guard->Z = objectZ + 0.5; // 决定平台gameobject的大小
    guard->O = objectO;
    ...
    }else 
    {
        ...
        if(QueryResult* queryResult = CharacterDatabase.PQuery("SELECT creatureName, creatureEntry, creatureCost FROM custom_td_base_stats WHERE creatureEntry = '%u'", entry))
        {
            ...
            }else
            {
                ...
                RecordLog("TowerDefense: 事件 ID: [%u] 没有正确的为: [%u] 产生 [%u] 守卫等等资源.", GetEventId(), guard->Entry,creatureCost);
            }  
        }
    }
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::UpgradeGuardMenu(uint64 guid)
{
    if(!guid)
        return;
    Player *player = GetPlayer();
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::UpgradeGuard(uint64 guid)
{
    if(!guid)
        return;
    Player* player = GetPlayer();
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SellGuard(uint64 guid)
{
    if(!guid)
        return;
    Player* player = GetPlayer();
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardList()
{
    Player* player = GetPlayer();
    if(!player)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SpawnFromGuardList(uint32 Action)
{
    Player* player = GetPlayer();
    if(!player || !Action)
        return;
...
MonsterInfo* TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetMonsterByEntry(uint32 Entry)
{
    if(!Entry)
        return NULL;
...
GuardInfo* TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardByGuid(uint64 guid)
{
    if(guid){
        if (Guards.find(guid) != Guards.end())
        {
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardResourceCost(uint32 entry)
{
    if(!entry)
        return 0;
...
bool TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardIsAntiAir(uint64 guid)
{
    if(guid){
...
bool TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardIsAntiGround(uint64 guid)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard)
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardSpellByCastType(uint64 guid, TDEventSpellCastType type)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard)
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardAttSpeed(uint64 guid)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard)
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardDamage(uint64 guid)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard){
...
float TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardAttackDistance(uint64 guid)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard)
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardUpgradeCost(uint64 guid)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard){
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetGuardSellPrice(uint64 guid)
{
    if(guid){
        GuardInfo* guard = GetGuardByGuid(guid);
        if(guard){
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::HandleEventComplete(TDEventCompleteType completeType)
{
    Player *player = GetPlayer();
    if(!player)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::UpdateHealth(uint32 remove)
{
    if(!remove)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::UpdateResources(TDEventUpdateType type, uint32 add)
{
    if(!type || !add)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::AddSimpleMenuItemToPlayer(Player* player, uint32 icon, uint32 sender,uint32 action, const char *Message, ...)
{
    if(!player)
        return;
    ...
    va_end(List);
    player->PlayerTalkClass->GetGossipMenu().AddMenuItem(icon,	Result,	sender,	action, "", false);
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SendMessageToPlayer(const char *msgToSend, ...)
{
    Player *player = GetPlayer();
    if(!player)
        return;
...
uint32 TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GenerateEventId()
{
    _highEventId++;
    return _highEventId;
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::UpdatePlayerStats(uint32 playerGUID, TDEventPlayerIndex statIndex, uint32 statIncreaseValue)
{
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SendMailToPlayer(Player *player, uint32 playerGUID, const char *mailMessage, ...)
{
    ...
    if(player){
        ...
        mailDraft.SendMailTo(MailReceiver(playerGUID), toSend);
        RecordLog("TowerDefense: Player GUID: [%u] received the mail: [%s] from the tower defense system.", playerGUID, Body);
    }
    CharacterDatabase.CommitTransaction();
}

void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::SetupEventData()
{
    Player *player = GetPlayer();
    if(!player)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::DeleteEventData()
{
    Player *player = GetPlayer();
    if(!player)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::GetRecords(Player *player)
{
    if(!player)
        return;
...
void TowerDefenseInstanceScript::TowerDefenseInstanceScriptAI::RecordLog(const char * str, ...)
{
    if (!str)
        return;
