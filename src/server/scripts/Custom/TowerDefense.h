...
struct MonsterInfo
{
    ...
    void Spawn(Creature* spawner)
    {
        ...
        Creature* me = spawner->SummonCreature(Entry, GetWaypointData(1).m_positionX, GetWaypointData(1).m_positionY, GetWaypointData(1).m_positionZ,0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000);
        if(!me)
...
public:
    ...
    struct TowerDefenseInstanceScriptAI : public ScriptedInstance
    {
       // TowerDefenseInstanceScriptAI(InstanceMap* map) :
        TowerDefenseInstanceScriptAI(Map* map) : ScriptedInstance(map)
        {
            Initialize();
        };
