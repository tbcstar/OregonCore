...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recv_data)
{
    ...
    sLog.outChar("账号: %d (IP: %s) 创建角色:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), name.c_str(), pNewChar->GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recv_data)
{
    ...
    sLog.outChar("账号: %d (IP: %s) 删除角色:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), name.c_str(), GUID_LOPART(guid));
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    std::string IP_str = GetRemoteAddress();
    sLog.outChar("账号: %d (IP: %s) 登录角色:[%s] (guid: %u)",
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(QueryResult* result, uint32 accountId, std::string newname)
{
    ...
    sLog.outChar("账号: %d (IP: %s) 角色:[%s] (guid:%u) 修改名字为: %s", session->GetAccountId(), session->GetRemoteAddress().c_str(), oldname.c_str(), guidLow, newname.c_str());
