    ...
    case CHAT_MSG_PARTY:
        {
            ...
            if (sWorld.getConfig(CONFIG_CHATLOG_PARTY))
                sLog.outChat("[PARTY] 玩家 %s tells group with leader %s: %s",
    ...
    case CHAT_MSG_OFFICER:
        {
            if (GetPlayer()->GetGuildId())
            {
                ...
                if (sWorld.getConfig(CONFIG_CHATLOG_GUILD))
                    sLog.outChat("[OFFICER] 玩家 %s tells guild %s officers: %s",
                                 GetPlayer()->GetName(), guild->GetName().c_str(), msg.c_str());
            }
        }
        break;
    case CHAT_MSG_RAID:
        {
            ...
            if (sWorld.getConfig(CONFIG_CHATLOG_RAID))
                sLog.outChat("[RAID] 玩家 %s tells raid with leader %s: %s",
                             GetPlayer()->GetName(), group->GetLeaderName(), msg.c_str());
        }
        break;
    case CHAT_MSG_RAID_LEADER:
    ...
    case CHAT_MSG_BATTLEGROUND:
        {
            ...
            if (sWorld.getConfig(CONFIG_CHATLOG_BGROUND))
                sLog.outChat("[BATTLEGROUND] 玩家 %s tells battleground with leader %s: %s",
                             GetPlayer()->GetName(), group->GetLeaderName(), msg.c_str());
        }
        break;
