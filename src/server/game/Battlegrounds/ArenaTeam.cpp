...
bool ArenaTeam::Create(uint64 captainGuid, uint32 type, std::string arenaTeamName)
{
    ...
    DEBUG_LOG("创建了新的竞技场队伍 [Id: %u] [类型: %u] [队长 GUID: " UI64FMTD "]", GetId(), GetType(), GetCaptain());
    return true;
}
...
bool ArenaTeam::AddMember(const uint64& playerGuid)
{
    ...
    if (pl)
    {
        ...
        DEBUG_LOG("玩家: %s [GUID: %u] 加入竞技场队伍类型: %u [Id: %u].", pl->GetName(), pl->GetGUIDLow(), GetType(), GetId());
    }
    return true;
}
...
void ArenaTeam::SetCaptain(const uint64& guid)
{
    ...
    if (Player* newcaptain = sObjectMgr.GetPlayer(guid))
    {
        ...
        DEBUG_LOG("玩家: %s [GUID: %u] 提升玩家: %s [GUID: %u] 为竞技场队伍队长 [Id: %u] [类型: %u].", oldcaptain->GetName(), oldcaptain->GetGUIDLow(), newcaptain->GetName(), newcaptain->GetGUIDLow(), GetId(), GetType());
    }
}
...
void ArenaTeam::DelMember(uint64 guid)
{
    ...
    if (Player* player = sObjectMgr.GetPlayer(guid))
    {
        ...
        DEBUG_LOG("玩家: %s [GUID: %u] 离开了竞技场队伍类型： %u [Id: %u].", player->GetName(), player->GetGUIDLow(), GetType(), GetId());
    }
...
void ArenaTeam::Disband(WorldSession* session)
{
    ...
    if (session)
        if (Player* player = session->GetPlayer())
            DEBUG_LOG("玩家: %s [GUID: %u] 解散竞技场队伍 类型：%u [Id: %u].", player->GetName(), player->GetGUIDLow(), GetType(), GetId());
